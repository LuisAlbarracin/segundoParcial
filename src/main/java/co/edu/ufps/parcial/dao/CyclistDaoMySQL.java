...
import co.edu.ufps.parcial.util.ConexionMySQL;

public class CyclistDaoMySQL implements CyclistDao {
	...
	private static final String SELECT_ALL_CYCLIST_SQL = "SELECT id, name, email, birthday, country, team FROM cyclist;";
	
	public CyclistDaoMySQL() {
	...
	public void insert(Cyclist cyclist) throws SQLException {
		...
		try {
			...
			preparedStatement.setDate(3, new java.sql.Date(cyclist.getBirthday().getTime()));
	...
	public List<Cyclist> selectAll() {
		...
		try {
			...
			while(rs.next()) {
				...
				cyclistes.add(new Cyclist(id, name, email, birthday, country, team));
			}
		}catch(SQLException e) {
			e.printStackTrace();
		}
